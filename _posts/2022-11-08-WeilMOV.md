...
**A.** A cryptographic algorithm is said to have a security level of $n$ bits if the best known attack can break it in $2^n$ steps. So if the naive brute force/exhaustive search is the best known attack for any algorithm the security level will be same as the key size in bits. However most asymmetric algorithms have much better attacks than the naive attack & hence their security level is lesser than the key size. For example, the Discrete Log problem on which Diffie-Hellman is based has a very powerful attack called Index Calculus, which can solve the Discrete Log problem in sub-exponential time. To compensate for this attack, we have to use very large key sizes in DH. To get 112 bit security, DH needs 2048 bit key size. 

Index Calculus uses smooth numbers. Likewise, the General Number Field Sieve for Integer Factorization using smooth numbers means RSA also requires a 2048 bit key size to get 112 bit security. Unlike the multiplicative group of a Finite Field, the group of Elliptic Curve points doesn't have a straightforward notion of smoothness & any methods similar to Index Calculus haven't been discovered in this group. Hence in Elliptic Curve Cryptography, you can get 112 bit security with just 224 bit Key Size.
...
The MOV (Menezes-Okamoto-Vanstone) attack transforms a Discrete Log problem in an Elliptic Curve Group into a Discrete Log Problem in the Multiplicative Group of a Finite Field - i.e. ECDLP to DLP. Since Index Calculus is possible in DLP, it may become easier to solve the DLP & thus solve the ECDLP.
...
$f_2 : U \times V \mapsto W \space$ ($f_2$ is a map from $U \times V$ to $W$)
...
Let's consider roots of unity in a finite field of prime order, say $\mathbb F_5 = \lbrace 0, 1, 2, 3, 4 \rbrace$. Every element in this field except for $0$ when raised to $4$ gives $1$. For e.g. $3^4 \bmod 5 \equiv 1$. So every element in the field $\mathbb F_5$ is a $4$-th root of unity. This is because for any non zero element $a \in \mathbb F_q$, we know that $a^q = a$ & hence $a^{q - 1} = 1$ by Fermat's Little Theorem. So every element $a$ in a finite field of prime order $q$ is a ($q-1$)-th root of unity. 
...
When the additional constraint $m \nmid (q - 1)$ is satisfied, the Weil pairing which is a map from $\mathbb G_1$ X $\mathbb G_2$ to the multiplicative group $\mathbb G_T$ can be constructed.

$e_m : \mathbb G_1 \times \mathbb G_2 \mapsto \mathbb G_T$ 
...
The right-hand side of the Weil Pairing bilinearity is multiplicative rather than additive, which is what we saw earlier with bilinear maps of vector spaces.

$\quad e_m(A_1 + A_2, B) = e_m(A_1, B).e_m(A_2,B)$

$\quad e_m(A, B_1 + B_2) = e_m(A, B_1).e_m(A,B_2)$

Let $\mathbb G_1$ & $\mathbb G_2$ be the generators of the 2 Elliptic Curve Groups. Let $\alpha$ & $\beta$ be scalars.
...
**3)** Compute $S = (\frac {n}{m}) T$. If $S = \mathcal O$, then go back to step 2 & choose another random point $T$. If it's not $\mathcal O$, then it's a point of order $m$ as shown below 
...
If the embedding degree $k$ is very large, transforming the ECDLP on $E(\mathbb F_q)$ to a DLP on $\mathbb F_{q^k}$ won't help. But if the embedding degree is small enough, then the DLP can become significantly easier. For e.g., a curve with a 256-bit $q$ usually offers 128 bits of security. But if it has an embedding degree $2$, then we can map the discrete logarithm to the field $\mathbb F_{q^2}$ which offers only 60 bits of security which can be broken by Index Calculus.
