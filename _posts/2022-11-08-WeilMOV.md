...
$E$ is an elliptic curve defined over a prime field $\mathbb F_q$. A point $P \in E(\mathbb F_p)$ satisfying $mP = \mathcal O$ is called a $m$-torsion point. The subgroup of all $m$-torsion points in $E(\mathbb F_p$ is denoted by $E(\mathbb F_p)[m] = \lbrace P \in E : mP = \mathcal O \rbrace$.
...
i.e. $A$ is a point of order $m$ in $E(\mathbb F_q)$ and $B$ is a point of order $m$ in $E(\mathbb F_{q^k})$. The map gives as output an *m*th root of unity $e_m(A,B)$. The output of the Weil Pairing when raised to $m$ always gives $1$ - i.e. ${e_m(A,B)}^m = 1$ for all $A, B \in E[m]$. 
...
If the embedding degree $k$ is very large, transforming the ECDLP on $E(\mathbb F_{q^k})$ to a DLP on $\mathbb F_{q^k}$ won't help. But if the the embedding degree is small enough, then the DLP can become significantly easier. For e.g., a curve with a 256-bit $q$ usually offers 128 bits of security. But if it has an embedding degree $2$, then we can map the discrete logarithm to the field $\mathbb F_{q^2}$ which offers only 60 bits of security which can be broken by Index Calculus.
