...
1. Choose a random point $T \in E(\mathbb F_{p^k})$ such that $T \notin E(\mathbb F_p)$   

1. Compute $S = (\frac {n}{m}) T$. If $S = \mathcal O$, then go back to step 2 & chose another random point $T$. If it's not $\mathcal O$, then it's a point of order $m$ as shown below 
...
1. Compute the Weil Pairing values 
...
1. If $p^k$ is not too large, then $u=v^r$ can be solved using Index Calculus & $r$ can be found. So we have solved $Q = rP$ 
