...
$\quad$ The Right Hand Side of the Weil Pairing bilinearity is multiplicative rather than the additive bilinearity we saw earlier with Bilinear Maps of Vector Spaces.

$\quad$ $e_m(A_1 + A_2, B) = e_m(A_1, B).e_m(A_2,B)$

$\quad$ $e_m(A, B_1 + B_2) = e_m(A, B_1).e_m(A,B_2)$

$\quad$ Let $G_1$ & $G_2$ be the generators of the 2 Elliptic Curve Groups. Let $\alpha$ & $\beta$ be scalars.
