...
$E$ is an elliptic curve defined over a prime field $\mathbb F_q$. A point $P \in E(\mathbb F_p)$ satisfying $mP = \mathcal O$ is called a $m$-torsion point. The subgroup of all $m$-torsion points is denoted by $E[m] = \lbrace P \in E : mP = \mathcal O \rbrace$.
...
If the embedding degree $k$ is very large, transfering the discrete logarithm to $\mathbb F_{q^k}$ won't help. But for some curves, the embedding degree is small enough and this enables the MOV attack. For e.g., a curve with a 256-bit $q$ usually offers 128 bits of security. But if it has an embedding degree $2$, then we can map the discrete logarithm to the field $\mathbb F_{q^2}$ which offers only 60 bits of security which can be broken by Index Calculus.
