...
- $G1 - all points in this subgroup are in the Curve over the base field $\mathbb F_p$. 

- $G2 - all points in this subgroup are in the Curve over the extension $\mathbb F_{p^k}$. These are points formed by the [Frobenius Endomorphism](https://en.wikipedia.org/wiki/Frobenius_endomorphism).  
...
When the additional constraint $m \nmid (q − 1)$ is satisfied, the Weil pairing is a map from $G1$ X $G2$ to the multipicative group $G_T$ .

$e_m : G1$ X $G2 \mapsto G_T$ 

The Weil Pairing is a map $e_m$ which takes as input a pair of $m$-torsion points $A, B$ such that $A \in E(\mathbb F_q)[m]$ and $B \in E(\mathbb F_{q^k})[m]$. 
i.e. $A$ is a point of order $m$ in $E(\mathbb F_q)$ and $B$ is a point of order $m$ in $E(\mathbb F_{q^k})$. The map gives as output an *m*th root of unity $e_m(A,B)$. The output of the Weil Pairing when raised to $m$ always gives $1$ - i.e. ${e_m(A,B)}^m = 1$. 
