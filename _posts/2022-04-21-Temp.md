...
Let $k$ be the embedding degree with respect to $m$. Consider the extension field $\mathbb F_{q^k}$. The multiplicative group of this extension field $\mathbb F^{\star}_{p^k}$ excludes the element $0$ and hence it's order is $q^k - 1$. Since $m$ divides the order of the multiplicative group, by the Fundamental Theorem of Cyclic Groups, it has a unique subgroup of order $m$. 

**The Steps:**   

1) Compute the order of the Elliptic Curve over the Extension field i.e. $n = \char"0023 E(\mathbb F_{q^k})$. Since the $m$-Torsion group of $E(\mathbb F_p)$, is a subgroup of $E(\mathbb F_{q^k})$, $m$ divides $n$
