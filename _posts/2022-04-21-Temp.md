...
i.e. $y^{\frac {p-1}{p_i^{n_i}}} = ({g^{x_i}})^{\frac {p-1}{p_i^{n_i}}} \bmod p$
...
Since $x_i$ is $\bmod 2^{n_i}$, then the max value of $x$ can only be $2^n −1$. The number $2^n$ in binary representation needs $n+1$ bits. So the number $2^n −1$ will need $n_i$ bits. Hence when we expand $x_i$ in base 2, we will have $n_i$ co-efficients from $0$ to ${n_i}−1$

For any base other than 2, we do it similarly

$x_i=\sum_{i=0}^{n−1} a_i{p_1}^i$ where $a_i \in {0,…,p−1}$

$x_i=a_0+ a_1p_1 + a_2{p_1}^2 +…$
...
Rasing both sides by $\frac {p-1}{p_1^{n_1}}$, we get  

$y^{\frac {p-1}{p_1^{n_1}}} \equiv {(g^{a_0+a_1{p_1}+a_2{p_1}^22+…})}^{\frac {p−1}{p_1}} \bmod p$ 

$y^{\frac {p-1}{p_i^{n_i}}} \equiv {(g^{a_0+ a_1{p_1}+ a_2{p_2}^2+…})^{\frac {p-1}{p_i^{n_i}}}} \bmod p$ [substituting $x_i$ by its expansion]

$y^{\frac {p-1}{p_i^{n_i}}} \equiv g^{a_0 \frac {p-1}{p_i^{n_i}}} . g^{a_1{p_1}\frac {p-1}{p_i^{n_i}}}.g^{a_2{p_1}^2 \frac {p-1}{p_i^{n_i}}}… \bmod p$ 


All the other terms after $g^{a_0 \frac {p-1}{p_1^{n_1}}} \bmod p$ have a coefficient which is a multiple of $p_1$. So after cancelling out the $p_1$ from the denominator, these terms are of the form $g^{k(p−1)}$. By *Fermat’s Little Theorem*, all these terms evaluate to 1 & can be removed from the expression.

We are then left with

$y^{\frac {p-1}{p_1^{n_1}}} \equiv g^{a_0 \frac {p-1}{p_1^{n_1}}} \bmod p$
...
-----
We continue from the earlier equation


$y^{\frac {p-1}{p_i^{n_i}}} \equiv g^{a_0 \frac {p-1}{p_i^{n_i}}} . g^{a_1{p_1}\frac {p-1}{p_i^{n_i}}}.g^{a_2{p_1}^2 \frac {p-1}{p_i^{n_i}}}… \bmod p$ 

If you look at the first term on the RHS - g^{a_0 \frac {p-1}{p_i^{n_i}}} - there are no unknowns in this term, so it can be calculated. Let $c = g^{a_0 \frac {p-1}{p_i^{n_i}}}$. Since $c$ is the generator of $\mathbb F_p$ raised to some value, $c$ belongs to $\mathbb F_p$ & has an inverse in the field.

$y^{\frac {p-1}{p_i^{n_i}}}.{c}^{-1} \equiv  g^{a_1{p_1}\frac {p-1}{p_i^{n_i}}}.g^{a_2{p_1}^2 \frac {p-1}{p_i^{n_i}}}… \bmod p$ 

$y_i \equiv {g_i}^{a_0+a_1{p_1}+a_2{p_1}^2+…} \bmod p$

$y \equiv g^{a_0} . g^{a_1{p_1} + a_2{p_1}^2+ …} \bmod p$

$g$ is a generator, so $g^{a_0}$ is an element of $\mathbb F(p)$. So $g^{a_0}$ will have an inverse in $\mathbb F(p)$. So we can multiply both sides by the inverse.
...
Start again with $y_i \equiv {g_i}^{x_i} \bmod p$  
