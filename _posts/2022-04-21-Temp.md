...
Order of $\mathbb F(p)=p−1=p_1^{n_1}.p_2^{n_2}.p_3^{n_3}…$
Since each of the prime powers ${p_i}^{n_i}$ divides the order of the group, by the Fundamental Theorem of Cyclic Groups, there will be a cyclic subgroup of order ${p_i}^{n_i}$ with the generator for the subgroup being $g^{\frac {p-1} {p_i^{n_i}}}$ 

Let's take the DLP & raise both sides by $\frac {p-1}{p_i^{n_i}}$ 

$y^{\frac {p-1}{p_i^{n_i}}}$ $\equiv$ $({g^x})^{\frac {p-1}{p_i^{n_i}}} \bmod p$
