...
If the embedding degree $k$ is very large (the same size as $q$), transfering the discrete logarithm to $\mathbb F_{q^k}$ won't help you. But for some curves the embedding degree is small enough and this enables the MOV attack. For e.g., a curve with a $256$-bit $q$ usually offers 128 bits of security (i.e. can be attacked using 2128 steps); but if it has an embedding degree $2$, then we can map the discrete logarithm to the field $\mathbb F_{q^2}$ which offers only 60 bits of security which can be easily broken by Index Calculus.
