...
Let's say Alice chooses an $a = 65$.
$a' = 65*56%88 = 32$

Let's take an element $B = 20$ from the prime order subgroup & check if $aB = a'B$
...
Let's check if $a' = 64$ satisfies our condition that for an element $B$ in the prime order subgroup, $aB = a'B$, but for a torsion element $T$, $a'T = identity$. 

Let's take $32$ as the prime order-element to test it with
