...
Let $m = g^{\frac {(p-1)a_0}{p_i^2}}$ ($a_0$ is now a known value).

Since $p_1^2}$ is a factor of the order of the group, g^{\frac {p-1}{p_i^2}}$  is a generator of a subgroup. Hence g^{\frac {(p-1)a_0}{p_i^2}}$ i.e $m$ is an element of the field & has an inverse $m^{-1}$ in the field.
