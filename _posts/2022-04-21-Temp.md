...
Consider the extension field $\mathbb F_{q^k}$. The multiplicative group of $F_q^k$ excludes the element $0$ & hence it's order is $q^k - 1$. $n$ divides the order of $\mathbb F_{q^k}$. So by the Fundamental Theorem of Cyclic Groups $\mathbb F_{q^k}$ has a unique subgroup of order $n$ .
