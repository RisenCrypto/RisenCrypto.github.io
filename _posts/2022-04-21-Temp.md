...
Consider the extension field $\mathbb F_{q^k}$. The multiplicative group of the field $\mathbb F^{*}_{p^k}$ excludes the element $0$ & hence it's order is $q^k - 1$. $n$ divides the order of $\mathbb F^{*}_{p^k}$. So by the Fundamental Theorem of Cyclic Groups, $\mathbb F^{*}_{p^k}$ has a unique subgroup of order $n$ .
