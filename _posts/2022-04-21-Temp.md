...
Next let's consider Extension Fields, fields of prime power order i.e. $\mathbb F_{p^c}$

The order of all non-zero $a \in \mathbb F_{p^c}$ is either $p^{c}-1$ or something which divides $p^{c}-1$.

Hence $a^{p^c - 1} = 1$ for all non-zero $a \in \mathbb F_{p^c}$

Hence every non-zero element in an extension field also is a ($p^{c}-1$)th root of unity.
...
- $G_2$ - all points in this subgroup are in $E(\mathbb F_{p^k})$ with none of them being in $E(\mathbb F_q)$. These are the points formed by the [Frobenius Endomorphism](https://en.wikipedia.org/wiki/Frobenius_endomorphism).  
