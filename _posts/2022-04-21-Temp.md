...
Consider the extension field $\mathbb F_{q^k}$. The multiplicative group of $F_q^k$ excludes the element $0$ & hence it's order is $q^k - 1$. So $n$ divides the order of $\mathbb F_{q^k}$, by the Fundamental Theorem of Cyclic Groups $\mathbb F_{q^k}$ has a unique subgroup of order $n$ .
