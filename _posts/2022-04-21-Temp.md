...
A cyclic group $G_{mn}$ of order $mn$ where $m$ & $n$ are coprime is isomorphic to a product group $G_m \times G_n$. So our ellipic curve group $G_{8p}$ can be looked at as $G_8 \times G_p$. The map for this isomorphism is

$\phi (a, b) = na + mb$ where $a \in G_p, b \in G_8$

This means any point $Q$ on the full curve of order $8p$ can be written as sum of 2 components i.e. the prime order component $P$ & a $8$-torsion component $T$ from the subgroup of order $8$ i.e $Q = 8P + pT$
...
The scalar should also leave $8a$ undisturbed. Any scalar of the form $11k + 1$ will do this. We can express this as
...
So multipling by the scalar $56$ is a torsion safe method of cofactor clearing.
