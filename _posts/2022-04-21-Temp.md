...
Let's take the DLP & raise both sides by $\frac {p-1}{p_i^{n_i}}$. For the sake of simplicity, let $r_i = p_i^{n_i}$
...
$r_i = p_i^{n_i}$, so $g^{r_i}$ is the generator for the subgroup of order $p_i^{n_i}$.
...
We simplified the equation after raising using the new variables $y_i$, $g_i$ & $x_i$ to show that now it's a DLP problem in the subgroup. While solving however, we will use $y$ & $g$ along with the new variable $x_i$

i.e. $y^{p_i^{n_i}} = ({g^{x_i}})^{p_i^{n_i}}$
...
$y_i \equiv g_i^{a_0+ a_1{p_1}+ a_2{p_2}^2+…} \bmod p$ [substituting $x_i$ by its expansion]

$y^{\frac {p−1}{p_1}} \equiv {(g^{a_0+a_1{p_1}+a_2{p_1}^22+…})}^{\frac {p−1}{p_1}} \bmod p$ [Substitution back values of $y_i$ & $g_i$
