...
**1) Reduce the DLP in the field into the DLPs in the prime power order subgroups**
...
Note: For e.g. if $p_i=2$, then we have to expand $x_$ in base 2 (i.e. do a binary expansion). If $x_i = 13$, then in binary (base 2), we can write it as the bitstring $1101$.
...
$y_i \equiv g^{a_0+ a_i{p_i}+ a_i{p_i}^2+…} \bmod p$ [substituting $x_i$ by its expansion]

$y^{\frac{p−1}{p_1}} \equiv (g^{a_0+a_1{p_1}+a_2{p_1}^2+…})^{\frac {p−1}{p_1}} \bmod p$ [Raise both sides to $\frac {p−1}{p_1}$]

${y_i}^{\frac {p−1}{p_1}} \equiv g^{(a_0+a_1{p_1}+a_2{p_1}^22+…)(\frac {p−1}{p_1})} \bmod p$
