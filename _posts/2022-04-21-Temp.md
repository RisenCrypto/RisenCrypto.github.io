...
Consider the extension field $\mathbb F_{q^k}$. The multiplicative group of the field i.e. $\mathbb F^{*}_{p^k}$ excludes the element $0$ and hence it's order is $q^k - 1$. $n$ divides the order of the multiplicative group. So by the Fundamental Theorem of Cyclic Groups, $\mathbb F^{*}_{p^k}$ 
