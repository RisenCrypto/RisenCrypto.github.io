...
### Implicit Openings and Blind Evaluation

We wanted to prove that the $QAP$ relationship $L(x) \cdot R(x) - O(x) = H(x) \cdot Z(x)$ holds true. In most modern SNARKs, the verifier provides a fresh random challenge point ($x = a$) to 'open' the commitment during the proof, but Groth16 does this differently. In Groth16, the random evaluation point ($x = \tau$) is chosen during the Trusted Setup, and its powers are hidden within the CRS as elliptic curve points. This means the evaluation point is decided once and for all when the circuit is deployed. The Prover then performs a blind evaluation of their QAP polynomials at this hidden point $\tau$.By constructing the proof elements using the CRS, the Prover is effectively proving that their polynomials satisfy the circuit's logic specifically at $\tau$. Because the Prover cannot see the raw value of $\tau$, they cannot 'cheat' the evaluation by crafting a fake polynomial that only works at that specific point. The Verifier then uses cryptographic pairings to confirm the math holds up without needing a separate, interactive opening phase - this is the primary reason why Groth16 is so fast and maintains such a small proof size. However, because of this Groth16 lacks universality: it requires a high-overhead Trusted Setup that must be completely rerun every time the circuit logic is modified.
