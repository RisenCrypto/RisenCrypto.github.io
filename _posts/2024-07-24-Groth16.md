...
Though Groth16 doesn't exactly use KZG Commitments, it uses something very similar. If you are not familiar which this type of Polynomial Commitment Schemes, you can go through this post on [KZG](/Kate/). It will introduce you to both Polynomial Commitment Schemes & Elliptic Curve Pairings.

We will use a Asymmetric Elliptic Curve Pairing setup

$e : \mathbb G_1 \times \mathbb G_2 \mapsto \mathbb G_T$

$G_1$ & $G_2$ are generators of cyclic groups $\mathbb G_1$ & $\mathbb G_2$ respectively.

The core properties of Elliptic Curve Pairings which we use here are the following

- $e(a G_1, b G_2) = e(b G_1, a G_2) = e(ab G_1, G_2) = e(G_1, ab G_2) = e(G_1, a G_2)^{b} = e(G_1, G_2)^{ab}$ 

where $a$ & $b$ are scalars

Let $y = a\cdot b + c \cdot d$


$e(yG_1,G_2)=e((ab+cd)G_1, G_2)$
...
So if we want to prove $y = a\cdot b + c \cdot d$, we can do so by proving

$e(yG_1,G_2) = e(G_1,G_2)^{ab}\cdot e(G_1,G_2)^{cd}$

### The Basic Idea

The Prover wants to prove to the Verifier that she knows the solution to a particular equation without revealing the solution itself. In order for her to to do this, the equation has to be first converted to Rank-1 Constraint System (R1CS)  & then to the Quadratic Arithmetic Program (QAP) form. I have covered the coversion here - [R1CS and QAP - From Zero to Hero](/2023-02-07-R1CSQAP).
...
For a polynomial $F(x)$, we will denote it's commitment in the Group $\mathbb G_n$ as $[F]_n$. (In sagemath code, we will denote it as CFn)
...
Our CRS will look like $CRS_i = \lbrace \tau^0 G_i, \tau^1 G_i, \tau^2 G_i, ... \rbrace$. We need to have as many powers as the degree of the highest polynomial we want to commit. The term $H\cdot Z$ (part of the polynomial $C$) has a degree $2n-2$ & for $G_1$, we would need powers upto $\tau^{2n-2} G_1$. For $G_2$, we only need upto $\tau^{n-1}\cdot G_2$ because we commit only $R$ which is of degree $n-1$ 
...
### Likewise calculate commitmsnets CR2 & CQ1
...
The subscript $p$ in $L_p$ is to denote that this part of each polynomial is known only to Prover, while subscript $v$ is to denote it's Public Input & known to the Verifier also. 

### Fixing Problems

During the Trusted Setup phase, 4 random elements are sampled - $\alpha, \beta, \gamma, \delta$. None of these are known to Prover or Verifier.

And when the prover generates the proof, he samples 2 more random elements $r_1$ & $r_2$ for randomising the proof.
...
$A B = L R+\beta L + r_2\delta L + \alpha R + \alpha \beta + \alpha r_2 \delta + r_1\delta R + r_1\delta \beta + r_1 r_2 \delta^2$

In the above, 

$-$ $LR = O + HZ$

$-$ $\beta L = \beta L_v + \beta L_p$ (Likewise $R$ & $O$ are also split)

We also divide some of the terms with $\gamma$ & $\delta$

We end up with 

$A B = \alpha\beta + I \gamma + C\delta$ 

where $C = \frac {\beta L_p + \alpha L_p + O_p + HZ} {\delta} + A r_2 + B r_1 - r_1 r_2 \delta$

and $I = (\frac {\beta L_v + \alpha L_v + O_v}{\gamma})$
...
Using the $y = ab + cd$ property of pairings we introduced in the pre-requisites section, verifier can verify if $A B \stackrel {?}{=} \alpha\beta + I \gamma + C\delta$ by checking if
...
To accomodate all the above changes, we have to add more elements to the trusted setup.
**Phase 1:**
...
Groth16 Trusted Setup has 2 phases - the above is Phase 1 or Universal Setup. If you create a Phase 1 setup of $n$ gates, it can be reused for any Circuit which has $n$ or less gates.
...
Now, we don't know $\tau$ or $Z(\tau)$, so how do we compute this list?
...
$Z(x)$ is of degree $n$ & the max degree of $x$ we multiply it by is $n-2$, so we require $\tau$ powers upto $2n-2$ in $CRS1$ to compute $CRShz$. 

So having $CRS1$ upto $2n-2$ enables us to split the Trusted Setup into 2 parts - Phase 1 which is the **Universal Setup** & Phase 2 which is the **Circuit Specific Setup**. Phase 1 can be reused for any other circuit as long it has $n$ or less gates.
...
(7474 : 3237 : 1)
~~~
The only thing to explain here is possibly the commitment of $H\cdot Z$
...
$-$ Prover multiplies $H$ & $Z$ & gets $P = H\cdot Z$ - the degree of $P$ is the degree of $H$ + degree of $Z$

Prover computes the commitment of $P$

$-$ First, we evaluate $Z$ at $x = \tau$ which gives us a value $z = Z(\tau)$
...
Verifier needs these commitments $[I]_1,[\alpha]_1, [\beta]_2, [\gamma]_2 [\delta]_2$
...
- Polynomials $L, R$ & $O$ are formed by multiplying the 3 $QAP$ matrices with the Solution Vector $S$. We have no way for confirming if she used consistent values from the same Witness part of $S$ in computing $L$, $R$ & $O$

    **Fix:** The shifting of $L$ & $R$ with $\alpha$ & $\beta$ ensures that $L, R$ & $O$ are consistent in using the same $S$ The product $\alpha\cdot\beta$ in the verification equation computed by the verifier guarantees that $A$ and $B$ involve non-trivial $\alpha$ and $\beta$ components. The paper presents a proof of how this means the product $A\cdot B$ involves a linear dependence on $\alpha$ and $\beta$, and this linear dependence can only be balanced out by a $C$ with a consistent choice of $S$ in all three of
$L, R$ & $C$ used to compute $A, B$ & $C$.
...
- We had no way of confirming that the Prover used the right polynomial $Z$ computing the commitment of $H\cdot Z$
    ...
    **Fix:** The term $C$ contains $\frac {H\cdot Z}{\delta}$ - however, the prover doesn't know the value of $\delta$. So if the prover has to compute the commitment using $CRShz$ which uses the $Z$. Any  commitment of $C$ which is sent by the Prover won't balance out the remaining terms (parts of which are computed by the verifier), unless he used $Z$
...
**Note:** We did everything in sagemath. If you are interested in a python implementation instead with explanations, you can check out Crypto Fairy's excellent series of [Blog Posts on Groth16](https://medium.com/coinmonks/under-the-hood-of-zksnark-groth16-protocol-2843b0d1558b)
