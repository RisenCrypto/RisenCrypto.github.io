...
Consider the following Circuit & its trace for witness $x = 4$![Circuit](/images/Circuit3.png?raw=true)
...
To check this, let's create a new vector $a'$ which contains pairs, each pair containing an element of $a$ and its index in $a$
...
Let's add the random number $\gamma$ to each element of our vectors $a'$ & $a''$
...
Now we can multiply all the elements of $a'$ with each other & then do the same for $a''$ & check if the 2 products are equal
...
This check will prove the permutation as per the map $\sigma$ is satisfied for our vector $a$.

To simplify the above example, we used gate numbers 1, 2, 3 etc. However, as [explained here](/$\mathcal{P} \mathfrak{lon}\mathcal{K}$WHY#multiplicative-subgroup), $\mathcal{P} \mathfrak{lon}\mathcal{K}$ uses elements of a multiplicative subgroup $H = \lbrace 1, \omega, \omega^2, \omega^3, ..., \omega^{n-1} \rbrace$ to number the gates
...
$i = n$ is the same as $i=0$ because we are operating in a multiplicative subgroup of order $n$.
...
- We have to also prove the above steps were followed in building $z(\omega)$ - i.e. the $z$'s were accumulated as computed above.
