...
Any of these extensions can also be reprsented by a different notation
...
Now, let's do something similar for construction extension fields. Consider the Polynomial Ring $\mathbb{F}_2[x]$, which consists of all polynomials with coefficients from the field $\mathbb{F}_2 = \lbrace 0, 1\rbrace$
...
All irreducible polynomials of degree $2$ & $3$ in $\mathbb F_2[x]$ are both irreducible & primitive.
...
Let's I construct $F_{2^4}$ by adjoining the root of $x^4 + x^3 + x^2 + x + 1$ - let's say the root is $\beta$ (I am using a diff label just for differentiating between the root of a primitive & non-primitive polynomial - you can use $\alpha$ also if you want).
