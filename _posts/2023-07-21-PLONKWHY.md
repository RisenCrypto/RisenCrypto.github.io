...
**A:** Older SNARKs like [Groth16](/R1CSQAP/), Pinocchio etc only verify the computation in each of the gates (Gate Constraints) but do not have something like Copy Constraints which verify the consistency across the gates. This is because the older SNARKs use a Witness vector(Solution vector) in verifying computation in each of the gates. A single, common Solution vector is used across all the gates. This automatically ensures the consistency across the gates which is what the Copy Constraints in PLONK is used to verify. 
...
Take a look at [PLONK's Permutation Check](/PLONKPerm/), especially how we define the [Permuation Polynomial](/PLONKPerm#the-permutation-polynomial).
...
---  
#### Split Polynomials
**Q:** Why does the PLONK protocol split the Quotient Polynomial into 3 polynomials?

**A:** In Round 3 (Page 29 of the PLONK Paper), the Quotient Polynomial $t(X)$ is split into 3 polynomials 
...
The opening time of a polynomial depends on the degree of the polynomial. However, having smaller polynomials may not help if the 3 polynomials are opened separately . The PLONK paper presents a technique for batched opening of different polynomials at different points (on Page 10) which ensures that the opening time of 3 smaller polynomis is much lower than the opening time of one polynomial of roughly 3 times the size. 
...
PLONK uses an optimisation which allows it to send one less evaluation proof. 
...
Commitement Reference String = $\lbrace G, aG,a^2G, a^3G, …, a^dG \rbrace$
...
$C_{G_a} = g_0\cdot G + g_1a\cdot G + g_2a^2\cdot G + … + g_da^d\cdot G$
...
Because $h_1$ has been replaced by $c$ in $L(X)$, $L$ doesn't contains a multiplication of two polynomials. Hence the verifier can compute the commitment of $L$ by himself as 

$C_L = c\cdot C_{h_2} - C_{h_3}$ 

where $C_L$, $C_{h_2}$ & $C_{h_3}$ are commitments of $L$, $h_2$ & $h_3$ respectively. 

Since the commitment to $L$ has been computed using comittments to $h_2$ & $h_3$, verifying if $L(r) =0$ & $h_1(r) = c$ would be enough for the verifier to verify that $h_1(r)\cdot h_2(r) - h_3(r) {=} 0$. The verifier doesnt need to send the evaluation proofs for $h_2$ & $h_3$. 
...
(In the PLONK paper, this is described as an optimisation suggested by Mary Maller).
