...
**A:** Let's say we have 4 polynomials â€“ $f_1$, $f_2$, $f_3$ & $f_4 \in \mathbb F_p[x]$ where the max degree of these polynomials is $d$ which is very, very small as compared to $p$.
...
If the $f_1$, $f_2$, $f_3$, $f_4$ are all 0 at some random value $r_2$, then by the Schwartzâ€“Zippel lemma, $f_1$, $f_2$, $f_3$, $f_4$ are all zero polynomials with very high probability because the maximum degree of these polynomials is very, very small as compared to $p$
