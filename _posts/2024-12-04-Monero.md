...
A function $C(y) = y G$ can also work as commitment for the number $y$ - here $G$ is a generator of an Elliptic Curve where the discrete log problem is hard. 
...
- Protocols/methods I use for converting inputs before hashing may not be the same used by Monero - but it's not relevant for  understanding the cryptographic concepts.
