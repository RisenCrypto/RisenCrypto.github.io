...
Let's try to build a simple commitment scheme from scratch to help Peggy. A hash function $H$ can work as a commitment. She can send the hash of $5$ i.e. $H(5)$ to Victor as a commitment. After the race is over & Horse #5 has won the race, Peggy tells Victor that this was the same as what she predicted. Victor can hash the number $5$ himself & compare it to hash sent by Peggy beforehand to check if she had in fact predicted it correctly. Such a commitment though binding isn't hiding. Since there are only 12 horses - even before the race is complete, Victor can hash all numbers from 1 to 12 & check which hash matches to find the horse chosen by Peggy. Also, in the next race, if Peggy again thinks Horse #5 will win & sends $H(5)$ as the commitment, Victor can compare with the earlier commitment & figure out that Peggy has sent a commitment to 5.

Instead, Peggy samples a random number $b$ called as a blinding factor & sends $H(5, b)$ & sends it to Victor (as mentioned earlier, both $5$ & $b$ will be converted to bytes & concatenated before hashing). Since $b$ can be a huge number, Victor can't brute force all combinations from 1 to 12 to figure out which was the number chosen by Peggy. Once the race is over, Peggy can disclose $5$ & $b$ to Victor & he can compute $H(5, b)$ to check if she had indeed chosen $5$. And for every new commitment, Peggy choses a new blinding factor even if she choses 5 in a future race, the commitment won't reveal the value to Victor till she reveals the new blinding factor.

A function $C(y) = y G$ can also work as commmitment for the number $y$ - here $G$ is a generator of an Elliptic Curve. 
...
Let Bob's View Private Key be $v_b$ & Spend Private Key be $s_b$ respectively. Then his View Public Key & Spend Public key are $V_b = v_b G$ & $S_b = s_b G$ respectively.
...
Now $C - C' = (b - b')G$ is a commitment to a $0$  amount because the $i H$ terms would cancel. Only the sender knows the $z = b-b'$ which is the private key for the zero commitment. Let $P_z = z G$
