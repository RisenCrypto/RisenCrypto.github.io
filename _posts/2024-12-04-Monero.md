...
Instead, Alice samples a random number $b$ called a blinding factor & sends $H(5, b)$ & sends it to Bob (as mentioned earlier, both $5$ & $b$ will be converted to bytes & concatenated before hashing). Since $b$ can be a huge number, Bob can't brute force all combinations from 1 to 12 to figure out which was the number chosen by Alice. Once the race is over, Alice can disclose $5$ & $b$ to Bob & he can compute $H(5, b)$ to check if she had indeed chosen $5$. And for every new commitment, Alice chooses a new blinding factor; even if she chooses 5 in a future race, the commitment won't reveal the value to Bob until she reveals the new blinding factor.
...
What if a person has multiple Private Keys & wants to do ring signatures proving proof of possession of all of them? One way is to do multiple rings, one for each private key - i.e. let's say you have 2 private keys, you create 2 rings each having its own dummy public keys. And create 2 separate ring signatures.
...
$V^i_b$ is the View Output Key of the receiver of the $i$-th output & Alice will compute the corresponding one time Public Address as 
