...
- In my examples, I use `sha3_256` hash function from `hashlib` (Monero uses Keccak‑256, which differs from the NIST SHA‑3 standard by using different padding).

#### Pedersen Commitments

You can look up just the [What is a commitment](/Kate/#what-is-a-commitment) part of this linked post to understand what a commitment is. 

Alice claims that she can predict the winner of a horse race in which 12 horses are competing. She thinks Horse #$5$ will win the race. But she doesn't want to tell Bob before the race is over. However, if she tells Bob only after the race is over, then he obviously won't believe that she predicted it beforehand. Hence she commits to the number before the race & opens the commitment after the race is over to prove that she did predict it correctly.
...
Instead, Alice samples a random number $b$ called a blinding factor & sends $H(5, b)$ to Bob (as mentioned earlier, both $5$ & $b$ will be converted to bytes & concatenated before hashing). Since $b$ can be a huge number, Bob can't brute force all combinations from 1 to 12 to figure out which was the number chosen by Alice. Once the race is over, Alice can disclose $5$ & $b$ to Bob & he can compute $H(5, b)$ to check if she had indeed chosen $5$. And for every new commitment, Alice chooses a new blinding factor; even if she chooses 5 in a future race, the commitment won't reveal the value to Bob until she reveals the new blinding factor.

A function $C(y) = y G$ can also work as a commitment for the number $y$ - here $G$ is a generator of an Elliptic Curve where the discrete log problem is hard. 
...
Pedersen Commitments solve this by having 2 Generators $G$ & $B$ such that the relation between $B$ & $G$ isn't known - i.e. nobody knows an $n$ such that $B = nG$. The 2nd generator $B$ is used for the blinding factor. Monero uses $B = H_p(G)$ to compute a 2nd generator which satisfies this requirement.
...
If Alice knows $c$ before she chooses $R$, then she can easily choose a random $s$ & an $R$ such that $R = sG + cP$. That's why for this proof to be sound, Bob should send $c$ only after Alice has sent him $R$. The problem with this however is that it is interactive. But, it can easily be made non-interactive by using Fiat-Shamir. Instead of Bob sending $c$, Alice herself computes $c = H_s(R)$. Since $c$ is computed from $R$, Alice cannot choose an $R$ beforehand such that $R = sG + cP$, so the proof is sound.
...
- Protocols/methods I use for converting inputs before hashing may not be the same as used by Monero - but it's not relevant for  understanding the cryptographic concepts.
- Many Schnorr implementations use  $s = r + cx$ but Monero uses $s = r - cx$ (and also adjusts verification accordingly) - not much of a difference between the two ways - except that the first is optimised for signing & Monero's way is optimised for verification. On a blockchain, each signature is verified many, many times & hence it may help to optimise verification over signing.
...
- Alice samples a random $s_{k+1}$ & computes $R_{k+1} = s_{k+1}G + c_{k+1}P_{k+1}$
...
- Verifier continues this all the way to $c_{n-1} = H_s(M, R_{n-2})$ and $R_{n-1} = s_{n-1}G + c_{n-1}P_{n-1}$
- Then $c_0 = H_s(M, R_{n-1} )$

If the $c_0$ computed by the verifier equals the $c_0$ sent by the signer, then the signature has verified.
...
def ring_sign(msg_, Pubs_, s_, k_, x_):
    ...
    # Overwrite the k'th s with a computed one
    sc[k_] = r - c*x_
...
What if a person has multiple Private Keys & wants to do ring signatures proving possession of all of them? One way is to do multiple rings, one for each private key - i.e. let's say you have 2 private keys, you create 2 rings each having its own dummy public keys. And create 2 separate ring signatures.
...
- Alice samples a random $s_{i,{k+1}}$ & computes $R_{i,k+1} = s_{i,k+1}G + c_{k+1}P_{i,k+1}$  for all $i \in \lbrace 0, ..., m-1 \rbrace$
- Alice computes $c_{k+2} = H_s(M, R_{0,k+1},R_{1,k+1}, ... , R_{m-1,k+1})$ 

- Alice continues this all the way to the $n^{th}$ key where she samples a random $s_{i, n-1}$ &  computes $R_{i,n-1}$ similarly for all $i \in \lbrace 0, ..., m-1 \rbrace$

- Alice computes $c_0 = H_s(M, R_{0,n-1},R_{1,n-1}, ... , R_{m-1,n-1})$

- Alice continues the same all the way up to sampling $s_{i, k-1}$ for all $i \in \lbrace 0, ..., m-1 \rbrace$ & computes $c_k$ 
...
- Verifier starts with $c = c_0$ from the signature.

- For each $j$ from $0$ to $n-1$:

  - Compute $R_{i,j} = s_{i,j}G + c P_{i,j}$ for all $i \in \lbrace 0, ..., m-1 \rbrace$

  - Compute $c = H_s(M, R_{0,j}, R_{1,j}, \dots, R_{m-1,j})$

- After the loop, if the final computed $c$ equals the $c_0$ sent by the signer, the signature is verified.
...
where the superscript $0, 1, ...$ etc is the output index - if a transaction has multiple outputs, then it's indexed as $0, 1, ...$ etc

$V^i_b$ is the View Public Key of the receiver of the $i$-th output & Alice will compute the corresponding one-time Public Address as 

$P^i_o = K^i_{dh}  G + S^i_b$ 

where $S^i_b$ is the Spend Public Key of the receiver of the $i$-th output. 

In BTC, every time a new mined block appears, your wallet scans the block and finds transactions sent to your address. In Monero, your wallet uses the $R$ of each new transaction to compute $P_o$ from it (using Bob's View Private Key) & checks if the transaction is sent to $P_o$ to find transactions meant for him.
...
When Bob wants to spend this output, he has to prove possession of $x_o$ - the View Private Key ($v_b$) & the Spend Private Key ($s_b$) both need to be known to know $x_o$ i.e. to spend the UTXO. So Bob can hand off his View Private Key to someone to delegate the responsibility of checking transactions without allowing them to spend it. 

In BTC, money is sent to non-secret addresses. In Monero, it is sent to a one-time address to preserve privacy and anonymity.
...
There are already commitments for all inputs (created by whoever sent those amounts to sender). Sender creates commitments for all outputs. She chooses blinding factors for all outputs except one randomly. That one is chosen explicitly to balance the sums. And that one is chosen explicitly such that the sum of the input blinding factors is equal to the sum of the output blinding factors.  

In BTC, nodes verify that the sum of input amounts is greater than or equal to the sum of output amounts. In Monero, because amounts are hidden in Pedersen commitments, nodes cannot directly check an inequality. Instead, the sender chooses blinding factors so that the sum of the input commitments equals the sum of the output commitments plus a commitment to the transaction fee, and nodes verify that equality. The actual fee amount is verified by the range proof and balance equation. 
...
- $Enc(val) = val\space  \bigoplus_8 \space b_v$

$Enc(val)$ & $C_v$ are known - they are present in the transaction data. So if an attacker knows (or can guess) the general range of a transaction (that it is between some amounts x to y XMR) - then they can try each of the values in the range to check if that fits in all 3 of the above equations. Domain Separation helps to prevent this.
...
The sender knows $b$ & $i$ as described in the [Amount Commitments](./#amount-commitments) section. The sender creates a new commitment for the same amount with a different blinding factor

$C'_i = i G + b'B$

This is called a pseudo commitment

Now $C_i - C'_i = (b - b')B$ is a commitment to a $0$  amount because the $i G$ terms would cancel out. Only the sender knows $z = b-b'$ which is the private key for the zero commitment. Let $P_z = z B$

We already have one Private Key ($x_o$) from the [Stealth Addresses](./#stealth-addresses) section which the signer has to prove possession of & this is now a second one. 
...
where $Pseudo_A$ & $Out_C$ are Pseudo Pedersen Commitment of Input A & Pedersen Commitment of Output C respectively & so on.
...
$KI = x_0\star hash_p(P_{0, k})$

$DKI_k = r_0\star hash_p(P_{0, k})$
...
$DKI_{k+1} = s_{0,k+1}\star hash_p(P_{0, k+1}) + c*KI$
...
The signature resulting from this operation is

$\lbrace KI, c_0, s_{0,0}, s_{0,1}, \dots, s_{0,n-1}, s_{1,0}, \dots, s_{m-1,n-1} \rbrace$
...
- For each $j$ from $0$ to $n-1$:
  ...
  - Compute $DKI_j = s_{0,j} \star H_p(P_{0,j}) + c \star KI$

  - Compute $c = H_s(M, R_{0,j}, R_{1,j}, \dots, R_{m-1,j}, DKI_j)$

- After the loop, if the final computed $c$ equals the $c_0$ sent by the signer, the signature is verified.


An additional check also needs to be done in Monero because unlike our example, Monero uses a Composite Order Curve. A signer could add a point on the Cofactor subgroup to the Key Image to get a new Key Image. And then create a signature with all $c_i$'s which are multiples of the order of the Cofactor Subgroup - this can be done by trial & error. This will give the signer a 2nd Key Image & Signature for the same UTXO which will verify correctly - thereby allowing double spending. Hence Monero validators check if the Key Image is a point on the main group of the elliptic curve, this is trivially checked by checking if $q KI \stackrel {?}{=}0$ 

A generalization of MLSAG could compute a Key Image for each one of the Private Keys in possession of the signer & publish a Key Image Vector instead of just a Key Image. However, in the Monero use case, it only makes sense to compute the Key Image for the 1st key - i.e. the  spend key. Hence we do not do a Key Image for the commitment to zero.

Below is the SageMath program for a signature for spending one of the input UTXOs of a transaction. Since each signature will always prove possession of 2 keys (One-time Address & Commitment to Zero), we will always have $m=2$. Monero also fixes that the first row of the matrix is the one-time Address Public Key & the 2nd row is the Commitment to Zero Key - so the Key Image is generated from the First Row.
...
def ring_multi_sign(msg_, Pubs_, s_, xs_):
    ...
    # After the loop, c is the challenge for index k.
    # Compute the signer's s values for column k only once.
    for i in range(m):
        sc[i][k] = r[i] - c * xs_[i]
...
- **rct_signatures** (though the name is rct_signatures, this field doesn't contain the signature which is actually stored in the field MGs)
    ...
    - **pseudoOuts** (list of all pseudo commitments for the transaction - one for each vin)
...
- **MGs** (list of signatures, one for each vin. Each signature contains the following fields)
