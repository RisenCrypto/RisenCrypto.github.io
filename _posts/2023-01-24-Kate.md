...
The Commitment Scheme tests the polynomials only at a small number of points ($a$ & the opening point). The Binding property of the commitment scheme is based on the Schwartz-Zippel Lemma.
...
Though $a$ has been deleted, the committer can still compute the commitment using the Reference String.
...
The proof that the committer provides to the verifier is the commitment of the Quotient Polynomial $Q(x)$ i.e. $C_Q$

$Q(x)$ is defined as 

$Q(x) = \frac {F(x) - F(b)} {x-b} = \frac {F(x) - c} {x-b}$

By **Little Bezout's Theorem**, if $F(x)$ is a polynomial, then $F(x) - F(b)$ is perfectly divisible by $(x-b)$ i.e. the remainder is $0$. So $Q(x)$ above is a polynomial i.e. it won't have any variable in the denominator or any negative exponents. Since $Q(x)$ is a polynomial, then the committer can calculate the commitment of $Q(x)$ using the Reference String as follows 

$C_Q = Q(a)\cdot G$ 

 If $F(x)$ weren't perfectly divisible by $(x-b)$, then $Q(x)$ would have a denominator & negative exponents & the committer wouldn't be able to evaluate it using just the Reference String & without knowing $a$.
...
Note that any pairing based equality check $e(A, B) \stackrel {?}{=} e(P,Q)$ can also be expressed in a different way.

$e(A, B) \stackrel {?}{=} e(P,Q)$

$e(A, B) \stackrel {?}{=} e((-1)\cdot (-P),Q)$

Again using the bilinearity property on the Right hand side, we get
...
Since the degree of $P(x)$ is $t$ & $R(x)$ is the remainder of dividing $F(x)$ by $P(x)$, the degree of $R(x)$ is less than $t$. Since the verifier knows evaluation of $R(x)$ at $t$ points, he can find $R(x)$ using Lagrange's Interpolation. So now $R(x)$ is known to the verifier. 
...
$F(a)\cdot G$ is the commitment of $F$ i.e. $C_F$. Likewise, 2 other terms above are also committments. So,
...
$C_h \cdot (a-z) \stackrel {?}{=} F - v \quad --- (Eq\space\space 2)$ 

Using pairings,

$e(C_h \cdot (a-z), G) \stackrel {?}{=} e(F - v, G)$

Using the bilinearity property, 

$e(C_h, (a-z)\cdot G) \stackrel {?}{=} e(F - v, G)$

$e(C_h, aG - zG) \stackrel {?}{=} e(F - v, G)$

$z$ is known, so $zG$ is known. $aG$ is known from the SRS. $F$ & $v$ are also computable. So all the terms in the above are known, and the verifier can check if the equality holds or not.
...
Verifier sends a random $r \in \mathbb F_p$, and Prover uses it to combine the above 2 equations in a linearly independent way so that they can be tested together. This trick is [explained here](/PLONKWHY#linear-independence).
